/*-----------------------------------------------------------------
- Team
-----------------------------------------------------------------*/
export default {

  // Main
  main: [
    {
      name: 'Capt. Hasmet OZ',
      position: 'Master Mariner - Average Adjuster',
      desc: 'Capt. OZ spent over 10 years at sea prior to joining an Istanbul based company of surveyors and adjusters where he managed a team of over 60 persons. In 2012 Capt. OZ joined Seven Seas & Co and has been accepted as a full member of the International Association of Average Adjusters in 2013. The office in Istanbul with both Capt. Oz and Capt. Dekeukelaere handle all hull and machinery, loss of hire and consultancy claims work for shipowners. Seven Seas & Co also proud itself of being the first fully Turkish manned average adjusting office',
    },
    {
      name: 'Marc Dekeukelaere',
      position: 'LL.M. MIIMS Sworn Average Adjuster - Managing Director',
      desc: 'Marc holds a master degree of Maritime Law and is a Deck Officer (STCW II/2 unlimited). A practicing member of the International, American, French and Belgian Associations of Average Adjusters, he speaks French, English, Dutch, Portuguese and as a Director handles all matters within the company. He is mostly active in condition surveys on sea going vessels and general and particular average adjusting as well as on major cargo claims. Marc holds a certificate from the Institute of Chartered Shipbrokers and is a marine insurance broker. He carries out ISM and ISPS audits as a lead auditor and is a nautical inspector for flag administrations.',
    },
    {
      name: 'António Marques',
      position: 'Eng. - Director',
      desc: 'A former military officer, Capt. António Marques manages the Angola office and carries out marine surveys for about 30 years. He is responsible for a team of over 50 surveyors in all ports of Angola and represents both local and international clients in the handling of marine claims. He is a member of the International Association of Average Adjusters, a lead auditor for ISM/ISPS and a Nautical Inspector for flag administrations.',
    },
    {
      name: 'Tharcisse Nkiba',
      position: 'MSc. Sworn Marine Surveyor - Director',
      desc: 'Tharcisse is managing the office in Matadi. He holds a Master Degree and is fluent in English, French and local dialects. Tharcisse is at the service of our customers in all the Democratic Republic of the Congo. He holds a Technical Cargo Surveyor diploma and was sworn in by the Matadi Court as official marine and cargo surveyor. He heads a team of about 10 persons.',
    },
    {
      name: 'Abed Tahiri',
      position: 'Average Adjuster - Director',
      desc: 'Abed is the general manager of our Casablanca office, he has over 35 years of experience in the handling of marine claims and is a member of the Association Mondiale de Dispacheurs (AMD) and also a known average adjuster in Morocco. Abed speaks French, Arabic and English.',
    },
    {
      name: 'Andreas Nicolaou',
      position: 'Bsc. Eng. Msc. Mar. Tech C. Eng. - Director',
      desc: 'A Marine Engineer he is the head of the marine surveying department. Andreas worked for an insurance broker and several ship managers in both technical and insurance positions. He is a Classification Society surveyor and an expert in ship registrations, ISM and ISPS audits and certifications and regulatory issues relating to shipping.',
    },
    {
      name: 'Andreas Philippou',
      position: 'Mar. Eng. - Director',
      desc: 'He is an engineer officer and a Classification Society surveyor. An expert in ship registrations with expertise in hull and machinery surveys having attended over 3500 vessels.',
    },
    {
      name: 'Janusz Fedorowicz',
      position: 'Average Adjuster',
      desc: 'Past Chairman of the International Association of Average Adjusters. He has over 50 years of experience and runs a company associated to ours.',
    },
    {
      name: 'Nicolas. Breithoff',
      position: 'Senior surveyor',
      desc: 'After having spent many years at sea, Mr. Breithof joined our team as a specialist on marine and non-marine surveys. He is fluent in French, English, German and Dutch and carries out surveys all around Europe. He is available to attend at short notice in any country where clients may need our services for both surveys and investigations.',
    },
    {
      name: 'Olivier Dalamel de Bournet',
      position: 'B.E.Sc.',
      desc: 'Olivier manages our French office. He handles marine cargo claims for the past 20 years and acts as a cargo surveyor specialised in liquid products.',
    },
    {
      name: 'Capt. Aleksandr Gladkiy',
      position: 'General Manager Moscow',
      desc: 'Capt. Gladkiy spent over 15 years at sea on various types of vessels and in 2007 joined an insurance company as marine expert. He then opened a marine surveying company and attended marine surveys and claims in Russia and worldwide. He joined the team of Seven Seas & Co and is in charge of the Moscow office.',
    },
    {
      name: 'Michal Kowalski',
      desc: 'Michal is a loss adjuster and cargo surveyor who joined us from another similar type of business. He is currently handling our cargo claims as a surveyor and expert in theft and fraud investigations. Mike is available to handle loss adjustments, theft and fraud cases worldwide.',
    },
    {
      name: 'Me. Oliver Toussaint',
      position: 'LL.M.  Attorney at Law',
      desc: 'An Attorney at Law to the Brussels Bar, he handles claims and legal matters from Brussels. He is active in maritime, insurance and commercial law. He speaks French, English and Spanish.',
    },
    {
      name: 'Alexandre Ludy',
      position: 'Adjuster',
      desc: 'Alexandre is a qualified Indonesian certified adjuster. Alexandre has over 20 year experience in Marine Surveys and investigation as well as marine related liability such as Freight Forwarder\'s Liability, Warehouseman\'s Liability and Bailees\' Liability. He holds Associate membership in the Australian and New Zealand Institute of Insurance and Finance (ANZIIF), Indonesian Certified Adjusting Practitioner (ICAP), Fellow of the International Federation of Adjusting Associations (FIFAA). He is the person in charge in Indonesia with our parent company Mandiri Ajusting.',
    },
    {
      name: 'Dimitri Mitskos',
      position: 'MSc. Eng.',
      desc: 'A Belgian Civil Engineer who carries out surveys with special expertise in machinery and electrical or electronic equipment. Dimitri speaks Greek, French and English.',
    },
    {
      name: 'Capt. André Bouchet',
      desc: 'A Master Mariner, he served in command of 550\'000 Dwt ULCC and 85\'000 cbm Gas tankers and on various other vessels. André is also a Court Surveyor and an expert in the operation of tanker vessels. He is based in France and is available to attend marine and cargo surveys and consultancy work.',
    },
    {
      name: 'Capt Gilles Essigan',
      position: 'M.N.Sc. LL.M.',
      desc: 'A Master in Nautical Sciences, Gilles also obtained a Master degree of Maritime Law from the University of Brussels. In 1995 after his training period in Belgium, France and on the "Jeanne d\'Arc" he returned to his home country, Ivory Coast. Gilles is in charge of the office in Abidjan.',
    },
  ],

  // Others
  others: [
    {
      name: 'J. Meunier',
      desc: 'A Pharmacist for over 25 years who specializes in claims related to chemicals and pharmaceuticals.',
    },
    {
      name: 'Dr. M. De Maegd',
      desc: 'A Doctor of Medicine who provides expert opinions. Fluent in Dutch, French and English.',
    },
    {
      name: 'Alexia Kyriakidou',
      position: 'LL.M.',
      desc: 'Alexia graduated in England with a LL.M. and then returned to Cyprus where she handles claims.\n',
    },
    {
      name: 'Sören Brödslev Olsen',
      position: 'M.S. Mech. Eng. EUR. ING.',
      desc: 'A Civil Engineer who carries out surveys with special expertise in aluminium products.\n',
    },
    {
      name: 'Mauro Santos',
      position: 'Senior surveyor - Manager',
      desc: 'He is the head surveyor in the Angola office and holds the Technical Cargo Surveyor diploma.\n',
    },
    {
      name: 'José Gomes',
      desc: 'A senior surveyor in the Luanda office.',
    },
    {
      name: 'Teodoro Malakias',
      desc: 'A chief surveyor in our Lobito office.',
    },
    {
      name: 'Bruno Lourenço',
      desc: 'A senior surveyor in the Angola office.',
    },
    {
      name: 'Antonio Quitonga',
      desc: 'A senior surveyor in our Namibe office.',
    },
    {
      name: 'Manuel Atanasse',
      desc: 'A senior surveyor in Luanda.',
    },
    {
      name: 'Rene Raul',
      desc: 'Surveyor at Cabinda.',
    },
    {
      name: 'Junior Kengue',
      desc: 'Surveyor at Soyo.',
    },
    {
      name: 'Bertand L. Kaya',
      desc: 'He is a sworn marine and cargo Surveyor at Pointe-Noire and handles claims and surveys in Congo.',
    },
  ]

}
